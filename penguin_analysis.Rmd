...
# Cleaning penguins_raw data and saving the cleaned data as a new csv, penguins_clean:
...
ggplot(penguins_clean, aes(x = species)) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(
      transform = ~ .,  
      # Transform the scale of flipper length
      
      name = "Flipper Length (mm)",  
      # Title of the secondary axis.
      
      breaks = seq(0, 260, by = 20),  
      # Set the number of incraments by which the axis increases by.
      
      labels = seq(0, 260, by = 20)  
      # Display labels for flipper length.
    )
  ) +
  theme_minimal() +
  ...
  theme(
    ...
    # Adjust the title of the right axis.
    
    axis.text.y.right = element_text(hjust = 1)  
    # Adjust the text position on the right axis.
  )
...
# Producing, and using, a function that cleans raw_penguins data and saves it as a separate csv file in the "data" folder: 
...
```{r, producing a scatter plot overlaying linear regressions for each species}
...
# Apply viridis color palette for 95% confidence intervals.
...
My partner suggested using source(here("functions","filename") to make my code more concise. This would be a minor addditon to my use of predefined functions (all of which are already saved in my "functions folder") reducing the need to define functions within the core script. Therefore, I agree with this improvement; sourcing functions would improve the coherency of my code, whilst allowing for greater modularity to be achieved. This modularity would further limits the likelihood and impacts of accidental alterations to code, further lessening the effect of errors.

Additionally, my partner recommended including an additional summary/format section to each function file stored within the "functions" folder, suggesting this would allow for the faster identification of the purpose of each function file. The relatability of functions to the core code, was something I too noted in my improvements; I agree to make code reproducible it is key that additional files should immediately describe their role. Inclusion of summary information allows for users to quickly navigate between functions, ensuring appropriate referral in the case of required adjustment. This is also in line with standard practice, meaning that inclusion of summary information would support a format that is more recognisable to other users, further advancing its navigability.

Overall, across this task I have learnt the importance of simplicity in creating reproducible code. Ensuring that any constituent information, crucial for the running of core code, is clearly labelled and their purpose clearly exclaimed. Also, key to this, is the explicit and clear outlining of what any constituent files do; this can be easily achieved through a clear README file. The inclusion of this detail in my repository and it's constituent files, appeared to enable easy navigation, crucial for ensuring relevant files are utilised appropriately.
