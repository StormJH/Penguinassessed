...
cleaning_penguin_columns <- function(penguins_raw) {
  penguins_raw %>%
    ...
    select(-"comments") %>%
    drop_na()  
# This will remove rows with any missing values
}
...
#writes cleaned data to a csv file saved in data file
...
# Changing column names and removing comments column
penguins_raw <- write_csv(penguins_raw, here("data", "penguins_raw.csv"))
...
explanatoryfigure <- ggplot(penguins_clean, aes(x = culmen_length_mm, y = culmen_depth_mm, color = species, shape = species)) +  
  geom_point(alpha = 1, position = position_jitter(width = 0.1, height = 0.1, seed = 160)) +  
...
```{r, results figure with p values for slope coefficients, warning=FALSE}

# Fit separate linear models by species, and extract relevant coefficients and p-values

slope_table <- penguins_clean %>% 
  group_by(species) %>% 
# Groups penguins_clean by species
  summarise(model = list(lm(culmen_depth_mm ~ culmen_length_mm, data = cur_data())),  
    ...
    # Extracts the slope coefficients for culmen length (the predictor variable) across the 3 species, these are stored as a list
...
resultsfigure <- ggplot(penguins_clean, aes(x = culmen_length_mm, y = culmen_depth_mm, color = species, shape = species)) +
  geom_point(alpha = 0.4, position = position_jitter(width = 0.1, height = 0.1, seed = 160)) +  
...
# Apply viridis color palette for 95% CIs
...
# Edit the slope_table (produced for explanatory figure) to enhance the readability of it's column titles 
...
#  Summmarise the output of this model
...
Z_stat_results <- data %>%
    ...
    summarise(
      correlation = cor({{var1}}, {{var2}}, use = "complete.obs"),
      n = n()) %>%
   ...
   species_list <- Z_stat_results %>% pull({{species_col}}) %>% unique()
   #code identifying all the unique species names in a data frame.
  
  Z_stat_results_comparison <- combn(species_list, 2, simplify = FALSE) %>%
    # Generates a list of all possible pairs of species from the list, ["Adelie", "Chinstrap"], etc...
    lapply(function(pair) {
      species_1 <- Z_stat_results %>% filter({{species_col}} == pair[1])
      species_2 <- Z_stat_results %>% filter({{species_col}} == pair[2])
    # Extracts species 1 and 2 from each pair ["Adelie"], ["Chinstrap"].
      z_statistic <- abs(species_1$z - species_2$z) / sqrt(species_1$se^2 + species_2$se^2)
      p_value <- 2 * (1 - pnorm(z_statistic))  
      
# Calculates the difference in z-statistic between every species pairs, calculating a two-tailed p-value corresponding with this difference.
...
# Generates a data frame with altered column titles and rounded numbers to enhance readability.
    bind_rows() 
...
```{r, testing assumptions of ANCOVA, warning=FALSE}
...
##### Figure 7: Test of the normality asssumption in Z-test


```{r, saving figures, warning=FALSE}
# Create a function for saving figures as png files
...
# Calling the function on selected figures:
...
Moreover, the use of annotations would allow effective identification of potential errors in the code, making it easier to correct potential discrepancies. However, this was irrelevant for my partner’s code, as it was fully operational. 
...
Despite these minor additions, I believe the altering of my partners figures would be easily achieved, due to the source code being highly modulated and informatively annotated. These factors support the quick identification of the specific function for each line of code, allowing for appropriate adjustment to enact a desired output. 

#### d) Reflect on your own code based on your experience with your partner's code and their review of yours. (300-500 words)

I was pleased that my code was fully functional across users, additionally that the README file and regular annotations allowed for the easy navigation of my code and it's folders. These features I believe are key for reproducibilioty of code, by clearly relating each line and section to its function. 

My partner suggested using source(here("functions","filename") to make my code more concise, this would allow prevent functions from needing to be referenced in the core code. I agree with this improvement, this would improve the coherency of code, allowing for greater modularity to be achieved, and ensuring that it runs in the most succinct way possible. Additionally, my partner recommended including an additional summary/format section to each function stored within the "functions" folder, suggesting this would allow for the faster identification of the purpose of each function file. The reliability of functions to code, was something I too noted in my improvements, and I agree to make code reproducible it is key that additional files clearly describe their role.  
